...
def lambda_init_fn(depth):
    ...
    return 0.2 + 0.6 * math.exp(-0.3 * depth)
    ...
    def __init__(self, dim, num_patches, depth, heads, dim_head, mlp_dim_ratio, dropout=0., stochastic_depth=0.):
        ...
        for i in range(depth):
            self.layers.append(nn.ModuleList([
                PreNorm(num_patches, dim, SpectralGatedDifferentialAttention(
                    dim,
                    num_patches,
                    heads=heads,
                    dim_head=dim_head,
                ...
                PreNorm(num_patches, dim, FeedForward(
                    dim,
                    num_patches,
                    hidden_dim=int(dim * mlp_dim_ratio),
                    dropout=dropout 
                ))
            ]))
