...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., is_LSA=False, eps=1e-12):
    ...
    def forward(self, x, mask=None):
        ...
        if mask is not None:
            ...
            attn = self.softmax(attn)
        epsilon = self.eps
        #print epsilon value for confirmation
        #print(f"Using epsilon value: {epsilon}")
        threshold = 0.0
        colsum = attn.sum(dim=-2, keepdim=True)
        attn = attn/(colsum + epsilon)
        rowsum = attn.sum(dim=-1, keepdim=True)
        attn = attn/(rowsum + epsilon)
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, input_resolution, num_heads, window_size=7, shift_size=0,
                 ...
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, is_LSA=False, eps=1e-12):
        super().__init__()
        self.dim = dim
        ...
        self.attn = WindowAttention(
            ...
            qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, is_LSA=is_LSA, eps=eps)
...
class BasicLayer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, depth, num_heads, window_size, 
                 ...
                 is_LSA=False, is_SPT=False, eps=1e-12):
        ...
        self.blocks = nn.ModuleList([
            SwinTransformerBlock(dim=dim, input_resolution=input_resolution,
                                 ...
                                 norm_layer=norm_layer, is_LSA=is_LSA, eps=eps)
...
class SwinTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 ...
                 use_checkpoint=False, is_LSA=False, is_SPT=False, eps=1e-12,
                 **kwargs):
        super().__init__()
        ...
        for i_layer in range(self.num_layers):
            ...
            layer = BasicLayer(
                               ...
                               norm_layer=norm_layer, is_LSA=is_LSA, is_SPT=is_SPT, eps=eps,
