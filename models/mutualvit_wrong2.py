...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        dfpre.to_excel("attention_matrix_before_softmax.xlsx", index=False) 
        dfpost.to_excel("attention_matrix_after_softmax.xlsx", index=False)
        colsum = attn.sum(dim=-2) 
        colsumdf = colsum[0].detach().cpu().numpy() 
        dfcols = pd.DataFrame(colsumdf) 
        dfcols.to_excel("sum_of_cols.xlsx", index=False)
        ...
        dfmask.to_excel("mask.xlsx", index=False)
        ...
        dfnormal.to_excel("normalized_attnetion_with_threshold.xlsx", index=False)
        ...
        dfattn.to_excel("final_attention_scores.xlsx", index=False) 
