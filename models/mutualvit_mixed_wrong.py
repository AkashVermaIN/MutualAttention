...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        #print("Max Epoch: ", self.epochs)
        #print("Current Epoch: ", self.current_epoch)
        #print("Args Epoch: ", self.args.current_epoch)
        
        #limit = self.epochs
        #limit = 20
        
#        if(self.args.current_epoch == limit -1 and self.forward_counter >= 1168):
        ...
        # --- START STABILITY FIX: Cast to float32 for critical calculations ---
        # Get the current precision (likely float16 due to autocast)
        current_dtype = attn.dtype
        
        # Cast necessary tensors to float32 for the division/masking
        attn_32 = attn.float()
        colsum_32 = colsum.float()
        # --- END STABILITY FIX ---
        ...
        # Use the float32 version for thresholding and masking
        mask = (colsum_32 > threshold).float()
        ...
        # Perform the potentially unstable calculation in float32
        normalized_attention = attn_32 / (colsum_32 + epsilon) * mask + attn_32 * (1 - mask)
        ...
        # --- START STABILITY FIX: Cast back to original precision (float16) ---
        attn = normalized_attention.to(current_dtype)
        # --- END STABILITY FIX ---
